...
extension ProteinRenderer {
    ...
    public func createSphereModel(protein: Protein) -> (vertexData: MTLBuffer?, atomTypeData: MTLBuffer?, indexData: MTLBuffer?) {
        ...
        guard let buffer = commandQueue?.makeCommandBuffer() else { return (nil, nil, nil) }

        // Set Metal compute encoder
        guard let computeEncoder = buffer.makeComputeCommandEncoder() else { return (nil, nil, nil) }
